...
from schemas.clothes import SizeType,Clothe,ClothePayload
...
async def create_clothe(size: SizeType, payload: ClothePayload, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    if user_id is None:
        raise HTTPException(status_code=403, detail="jwt_token is invalid")
    clothe = cl.create_clothe_by_payload(db, user_id, payload.image_url, size, payload.color)
    return clothe
